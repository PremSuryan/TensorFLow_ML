...
def main(args):
    ...
    if choice == "Data information":
        ...
        **predicting the type of transport vehicle from an image** (between 15 categories) in order to identify 
        ...
        path = sorted(os.listdir("C:\Users\Lenovo\Downloads"))
        for each in path:
            st.image(mpimg.imread(f"/home/christian/Downloads/home/{each}"), width=None)

import sys
if __name__ == "__main__":
    sys.argv = ['main_script.py', '--num_classes', '15', '--model', 'model', '--train', './data/raw/vehicle/train/train/', '--test', './data/raw/vehicle/test/proof/', '--IMG_HEIGHT', '224', '--IMG_WIDTH', '224', '--batch_size', '32', '--epochs', '100', '--weight_decay', '1e-4']
